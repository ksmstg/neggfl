...
##' @param gamma2 The hyper-parameter control the degree of fusion.
...
negfl <- function(x, y, lambda2, gamma2, maxiter=1e5, burnin=3e4, scaled=T, method="mode_kde"){
  ...
  hparams = data.frame(lamb2=lambda2, gamma2=gamma2)
  B = maxiter
  posterior = negfl_bas(as.matrix(y), as.matrix(x), hparams, B)
